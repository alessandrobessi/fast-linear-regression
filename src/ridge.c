...
void ridge(const char x_file_name[], const char y_file_name[], const double lambda, const bool verbose,
    ...
    gsl_matrix *I = gsl_matrix_alloc(num_features, num_features);
    gsl_matrix_set_identity(I);
    gsl_matrix_scale(I, lambda);
    gsl_matrix_transpose_memcpy(XT, X);

    gsl_blas_dgemm(CblasNoTrans, CblasNoTrans, 1.0, XT, X, 0.0, XTX);
    gsl_matrix_add(XTX, I);
